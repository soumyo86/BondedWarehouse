...
func (t *CBDoc) viewItemListByDocumentId(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for row := range rows {		
		...
		newApp.Name = row.Columns[1].GetString_()
		newApp.Quantity = row.Columns[2].GetString_()
		newApp.Description = row.Columns[3].GetString_()
		newApp.Weightvolume = row.Columns[4].GetString_()
