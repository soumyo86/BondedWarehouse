...
func (t *CBDoc) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	err = stub.CreateTable("ItemTracker", []*shim.ColumnDefinition{
		...
		&shim.ColumnDefinition{Name: "estimatedTime", Type: shim.ColumnDefinition_STRING, Key: false},
	})
	if err != nil {
...
if len(args) != 14 {
		...
		ok, err = stub.InsertRow("ItemTracker", shim.Row{
			Columns: []*shim.Column{
				...
				&shim.Column{Value: &shim.Column_String_{String_: estimatedTime}},
			}})
...
func (t *CBDoc) deleteLineItem(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	// Get the row pertaining to this itemId
		var columns1 []shim.Column
		col1 := shim.Column{Value: &shim.Column_String_{String_: itemId}}
		columns1 = append(columns1, col1)

		row, err := stub.GetRow("ItemDetails", columns1)
		if err != nil {
			return nil, fmt.Errorf("Error: Failed retrieving records with item id %s. Error %s", itemId, err.Error())
		}
...
func (t *CBDoc) viewTruckDetailsByDocId(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for row := range rows {		
		...
		newApp.EstimatedTime = row.Columns[7].GetString_()
...
func (t *CBDoc) updateRejectedDocument(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	// Get the row pertaining to this asnNumber
		...
		estimatedTime := row.Columns[7].GetString_()
	...
	// Get the row pertaining to this truckId
		...
		ok, err := stub.InsertRow("ItemTracker", shim.Row{
			Columns: []*shim.Column{
				...
				&shim.Column{Value: &shim.Column_String_{String_: leavingCustom}},
				&shim.Column{Value: &shim.Column_String_{String_: receivedWarehouse}},
				&shim.Column{Value: &shim.Column_String_{String_: estimatedTimeIp}},
			}})
	...
	for row := range rows {		
		...
		if newApp.ArrivingCustom == arrivingCustom{
			...
			for row1 := range rows1 {
				...
				if newApp1.DocumentId == newApp.DocumentId && newApp1.Status != "Approved"{
					if  newApp1.Status != "Received"{
...
func (t *CBDoc) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "deleteLineItem" {
		t := CBDoc{}
		return t.deleteLineItem(stub, args)	
	}
